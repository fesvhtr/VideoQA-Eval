...
def cal_bleurt(gt,sub):
    ...
    checkpoint = "/home/dh/zsc/data/eval/BLEURT-20"
    references = [gt]
    candidates = [sub]
...
if __name__ == '__main__':
    sub = ' In the video, a man is seen taking something out of a car whose glass is shattered. This is an unusual event as it is not common for someone to take something out of a car with a shattered window. The man is seen walking away from the car, and the video ends with the car driving away.'
    gt = 'In the video, a man wearing a black mask first leans on the car window to observe the interior, then retrieves a black package from the shattered rear window of a white car and places it in his own car. He then drives up to a black car, exits the vehicle, shatters the driver\'s side window, retrieves two black packages from inside, and drives away'
    print('BLEU-4: ',calculate_bleu4(gt,sub))
    print('ROUGE-L: ',calculate_rouge(gt,sub))
    print('MoverScore: ',calculate_mover(gt,sub))
    print('BLEURT: ',cal_bleurt(gt,sub))
    evaluator = get_evaluator('fact')
    print('UniEval: ',calculate_UniEval(gt,sub,evaluator))
